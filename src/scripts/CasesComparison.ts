 ...
 * Worst Case (Malicious User highly connected)
 * Best Case (Malicious User Badly connected)
 *
 * PROTOCOL:
 * - Worst case:
 *  - Register N user
 *  - Following according to the pareto distribution
 *  - Manually set Top connected user's behaviour as MALICIOUS
 ...
 *  - Register N user
 *  - Following according to the pareto distribution
 *  - Manually set Top connected user's behaviour as Truthfull
 *  - Post X content for each user
 *  - I * ( Fetch & retweet )
 *
...
// EXPORT RESULTS
fs.mkdir('results', () => {
  jsonfile.writeFile('results/Witness_CaseComparisonTopConnected.json', topConnected, { spaces: 2, EOL: '\r\n' });
  console.log('\x1b[36m%s\x1b[0m', 'Top Connected Case comparison file written');

  jsonfile.writeFile('results/Witness_CaseComparisonAverageConnected.json', averageConnected, { spaces: 2, EOL: '\r\n' });
  console.log('\x1b[36m%s\x1b[0m', 'Average Connected Case Comparison file written');

  jsonfile.writeFile('results/Witness_CaseComparisonLessConnected.json', lessConnected, { spaces: 2, EOL: '\r\n' });
